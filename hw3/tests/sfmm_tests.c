...
Test(sf_memsuite_mine, realloc_same_block, .init = sf_mem_init, .fini = sf_mem_fini) {
	void *x = sf_malloc(sizeof(int));
	void *z = x;
	void *y = sf_realloc(x, sizeof(int));

	cr_assert(z==y, "Realloc'ed block pointer should be the same!");

	sf_header *header = (sf_header*)((char*)y - 8);
	cr_assert(header->block_size == 2, "Realloc'ed block size not what was expected!");
...
Test(sf_memsuite_mine, realloc_zero, .init = sf_mem_init, .fini = sf_mem_fini) {
	void *x = sf_malloc(sizeof(int));
	sf_header* header = (sf_header*)(x-8);
	int header_size = header->block_size<<4;
	void *y = sf_realloc(x, 0);

	cr_assert_null(y, "y should be NULL!");

	free_list *fl = &seg_free_list[find_list_index_from_size(header_size)];
	cr_assert(fl->head == NULL, "Head should be freed!");
}

Test(sf_memsuite_mine, free_random, .init = sf_mem_init, .fini = sf_mem_fini) {
	double *w = sf_malloc(LIST_1_MIN);
	double *x = sf_malloc(LIST_2_MIN);
	double *y = sf_malloc(LIST_3_MIN);
	double *z = sf_malloc(LIST_4_MIN);
	...
	sf_header* header = (sf_header*)w-1;
	sf_header* header2 = (sf_header*)x-1;
	// sf_header* header3 = (sf_header*)y-1;
	sf_header* header4 = (sf_header*)z-1;

	// cr_assert((header)->block_size << 4 == 0, "Should be 0");
	cr_assert((header)->block_size << 4 == 48, "Should be 48");
	cr_assert((header2)->block_size << 4 == 704, "Should be 704");
	cr_assert((header4)->block_size << 4 == 3344, "Should be 3344");

	free_list *fl = &seg_free_list[find_list_index_from_size(48)];
	cr_assert(fl->head->header.block_size<<4 == 48, "Head should be 48 block size");
	fl = &seg_free_list[find_list_index_from_size(704)];
	cr_assert(fl->head->header.block_size<<4 == 704, "Head should be 704 block size");
	fl = &seg_free_list[find_list_index_from_size(3344)];
	cr_assert(fl->head->header.block_size<<4 == 3344, "Head should be 3344 block size");
	fl = &seg_free_list[find_list_index_from_size(160)];
	cr_assert(fl->head == NULL, "Head should be NULL");
}

Test(sf_memsuite_mine, prev_malloc, .init = sf_mem_init, .fini = sf_mem_fini) {

	double *currBlock = sf_malloc(4);
	sf_malloc(4);
	double *newBlock = sf_malloc(4);
	sf_malloc(4);
	...
	cr_assert(fl->head->next->prev != NULL, "There should be a prev item!");
}

Test(sf_memsuite_mine, splinter_malloc, .init = sf_mem_init, .fini = sf_mem_fini) {

	double *currBlock = sf_malloc(4060);
	sf_header* header = (sf_header*)currBlock-1;

	cr_assert(header->block_size<<4 == 4096, "It's not 4096. You have to keep splinter!");
}